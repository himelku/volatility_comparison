...
if "prediction" in garch_results.columns:
    garch_results.rename(columns={"prediction": "predicted_volatility_garch"}, inplace=True)

# ------------------ Load VIX ------------------ #
vix = pd.read_csv(os.path.join(data_dir, "vix_15min.csv"))
vix["Date"] = pd.to_datetime(vix["timestamp"])
vix = vix.rename(columns={"Close": "Close_vix"} if "Close" in vix.columns else {"close": "Close_vix"})
vix.drop(columns=["timestamp", "Open", "High", "Low", "Adj Close", "Volume"], errors="ignore", inplace=True)

# ------------------ Merge ------------------ #
merged = pd.merge(spy_lstm, garch_results[["Date", "predicted_volatility_garch"]], on="Date", how="left")
merged = pd.merge(merged, spy_raw[["Date", "ewma_volatility"]], on="Date", how="left")
merged = pd.merge_asof(merged.sort_values("Date"), vix.sort_values("Date"), on="Date", direction="backward")

# Clean and prepare
essential_cols = ["volatility", "predicted_volatility_garch", "Close_vix", "ewma_volatility"]
merged.dropna(subset=essential_cols, inplace=True)

# ------------------ Features & Target ------------------ #
feature_columns = [col for col in merged.columns if col not in ["volatility", "Date", "timestamp"]]
...
X, y = create_dataset(merged[feature_columns], merged[target_column].values.reshape(-1, 1), time_steps)
input_shape = (time_steps, X.shape[2])

model_save_path = os.path.join("lstm_garch", "lstm_garch_ewma_vix.weights.h5")
...
for i in range(len(merged) - initial_train_size - validation_size - 1):
    ...
    results.append({
        ...
        "validation_end": merged["Date"].iloc[i + initial_train_size + validation_size + time_steps - 1],
        "test_date": merged["Date"].iloc[i + initial_train_size + validation_size + time_steps],
